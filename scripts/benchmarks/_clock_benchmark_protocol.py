"""Strict evidence contract for clock/timestamp CPU characterization."""
...
SCHEMA_ID = "rclcppyy.clock-benchmark/v1"
SAMPLE_SCHEMA = "rclcppyy.clock-sample/v1"
VARIANTS = (
    "stock-rclpy", "compatible-rclcppyy", "direct-cpp-rclcppyy",
    "native-orchestrated")
WORKLOADS = ("now", "now-nanoseconds")
...
OPERATION_ROUTES = {
    "stock-rclpy": {
        ...
        "now-nanoseconds": "rclpy.time.Time.nanoseconds",
    },
    "compatible-rclcppyy": {
        "now": "rclpy.clock.Clock.now",
        "now-nanoseconds": "rclpy.time.Time.nanoseconds",
    ...
    "direct-cpp-rclcppyy": {
        "now": "rclcppyy.direct_clock.DirectClock.now",
        "now-nanoseconds": (
            "rclcpp_kit.native_clock.NativeNodeClock.now_nanoseconds"
            " (via DirectClock's retained native clock)"),
    },
    "native-orchestrated": {
        "now": "rclcpp_kit.native_clock.NativeNodeClock.now",
        "now-nanoseconds": "rclcpp_kit.native_clock.NativeNodeClock.now_nanoseconds",
    },
}
...
def validate_sample(sample, *, warmup, operations, repetition, order_index):
    if not isinstance(sample, dict) or sample.get("schema") != SAMPLE_SCHEMA:
        raise ValueError("unsupported clock sample schema")
    ...
    if not _positive_integer(correctness.get("final_value")):
        raise ValueError("sample final timestamp was not verified")
    ...
    if variant in ("stock-rclpy", "compatible-rclcppyy"):
        if backend.get("clock_representation") != "rclpy.clock.Clock":
            raise ValueError("stock/compatible clock representation is invalid")
        if backend.get("exact_cpp_clock") is not False or backend.get(
                "cpp_clock_address") is not None:
            raise ValueError("stock/compatible lane claimed exact C++ clock storage")
    else:
        if backend.get("exact_cpp_clock") is not True or not _positive_integer(
                backend.get("cpp_clock_address")):
            raise ValueError("native-backed lane lacks exact C++ clock evidence")
        expected_representation = (
            "rclcppyy.direct_clock.DirectROSClock"
            if variant == "direct-cpp-rclcppyy"
            else "rclcpp_kit.native_clock.NativeNodeClock")
        if backend.get("clock_representation") != expected_representation:
            raise ValueError("native-backed lane representation identity is invalid")
...
def build_document(*, environment, command, warmup, operations, repetitions, samples):
    document = {
        ...
        "benchmark": {
            "name": "clock-timestamp-cpu-characterization",
...
def validate_document(document):
    if not isinstance(document, dict) or document.get("schema") != SCHEMA_ID:
        raise ValueError("unsupported clock benchmark schema")
    ...
    if not isinstance(benchmark, dict) or benchmark.get(
        ...
        raise ValueError("clock benchmark claims must remain disabled")
    ...
    if benchmark.get("name") != "clock-timestamp-cpu-characterization" or benchmark.get(
