#!/usr/bin/env python3
"""Run fixed-order stock/direct/native clock-timer + rate-sleep CPU samples."""
...
WORKER = HERE / "clock_timer_benchmark_worker.py"
...
def _arguments(argv=None):
    ...
    parser.add_argument("--warmup-operations", type=int, default=500)
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=REPO_ROOT / "build" / "clock-timer-characterization.json",
    )
...
def _print_summary(document, output):
    print("clock-timer + rate-sleep CPU characterization (claims disabled)")
    ...
    for workload, values in document["summary"]["median_cpu_ratios"].items():
        print(
            "%s: compatible/stock=%.4f direct/stock=%.4f native/stock=%.4f" % (
                workload,
                values["compatible_over_stock_process_cpu"],
                values["direct_over_stock_process_cpu"],
                values["native_over_stock_process_cpu"],
            ))
