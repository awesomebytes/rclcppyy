#!/usr/bin/env python3
"""Run rotated stock/direct remote parameter CPU samples."""
...
WORKER = HERE / "remote_parameter_benchmark_worker.py"
...
def _arguments(argv=None):
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=REPO_ROOT / "build" / "remote-parameter-characterization.json",
    )
...
def _environment(domain_id, args):
    ...
    metadata["benchmark_controls"] = {
        ...
        "init_jit_graph_setup_excluded": True,
...
def _print_summary(document, output):
    print("remote parameter CPU characterization (claims disabled)")
    print("primary metric: %s" % PRIMARY_METRIC)
    print("CPU scope: combined server/client process")
    print("raw evidence: %s" % output)
    for workload, values in document["summary"]["median_ratios"].items():
        print(
            "%s: direct/stock CPU=%.4f p50=%.4f p99=%.4f throughput=%.4f" % (
                workload,
                values["direct_over_stock_process_cpu"],
                values["direct_over_stock_latency_p50"],
                values["direct_over_stock_latency_p99"],
                values["direct_over_stock_throughput"],
            ))
