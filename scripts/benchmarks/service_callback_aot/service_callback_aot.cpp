...
constexpr const char * kPrefix = "@@RCLCPPYY_SERVICE_CALLBACK_V1@@";
...
class StagedServer final : public rclcpp::Node {
public:
  StagedServer(const std::string & name, const std::string & service_name)
...
int run_server(int argc, char ** argv)
{
  ...
  auto server = std::make_shared<StagedServer>(node_name, service_name);
  ...
  std::cout << kPrefix
            ...
            << "\"run_token\":" << quote(token) << ",\"pid\":" << getpid() << ","
            << "\"process_group_id\":" << getpgrp() << ",\"warmup_requests\":"
            << warmup << ",\"cpu_clock\":\"CLOCK_PROCESS_CPUTIME_ID\"}" << std::endl;
  ...
  std::cout << kPrefix
            ...
            << "\"run_token\":" << quote(token) << ",\"warmup_requests\":" << warmup << ","
            << "\"total_requests\":" << total << ",\"measured_requests\":" << measured << ","
            << "\"true_total\":" << true_total << ",\"true_measured\":" << true_measured << ","
            << "\"response_checksum\":" << checksum << ","
            << "\"python_callback_count_total\":0,\"python_boundary_crossings_measured\":0,"
            << "\"exceptions\":0,\"pending_requests\":0,\"cpu_time_ns\":"
...
int run_client(int argc, char ** argv)
{
  ...
  std::cout << kPrefix
            << "{\"schema\":\"rclcppyy.service-callback-client-event/v1\","
            << "\"event\":\"warmed\",\"run_token\":" << quote(token) << ","
            << "\"pid\":" << getpid() << ",\"process_group_id\":" << getpgrp() << ","
            << "\"warmup_requests\":" << warmup << ",\"loaded_rmw\":"
  ...
  std::cout << kPrefix
            ...
            << "\"messages\":" << messages << ",\"response_checksum\":" << checksum << ","
            ...
            << "\"server_node\":" << quote(server_name) << ",\"service_name\":"
            << quote(service_name) << ",\"service_type\":\"std_srvs/srv/SetBool\","
            << "\"pending_requests\":0,\"latency_ns\":[";
  ...
  const auto gone_deadline = std::chrono::steady_clock::now() + std::chrono::seconds(15);
...
{
  if (argc < 2) {
    std::cerr << "usage: service_callback_aot server|client ...\n";
    return 2;
  }
  try {
    ...
    std::cerr << "service callback AOT error: " << error.what() << '\n';
    return 2;
  }
}
