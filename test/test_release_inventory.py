...
def _fixture(tmp_path, architecture="x86_64"):
    ...
    definitions = [
        ("cppyy-kit", "0.2.0", "py_fixture_0", "noarch", ()),
        ("ros-jazzy-rclcpp-kit", "0.2.0", "py_fixture_0", "noarch", ()),
        (
            "ros-jazzy-rclcppyy",
            "0.3.0",
            "h_fixture_0",
    ...
    if architecture != "x86_64":
        definitions.append(("cppyy", "3.5.0", "py312_fixture_0", platform, ()))
    metadata = {}
    artifacts = []
    ...
    return attestation, suite_lock, extract


def _build(tmp_path, architecture="x86_64"):
    attestation, suite_lock, extractor = _fixture(tmp_path, architecture)
    ...
    return value, attestation, suite_lock


def test_inventory_validates_exact_x86_stack_and_emits_file_level_spdx(tmp_path):
    value, _attestation, _suite_lock = _build(tmp_path)
...
def test_inventory_includes_native_arm_bridge_as_exact_dependency(tmp_path):
    value, _attestation, _suite_lock = _build(tmp_path, "aarch64")
...
def test_inventory_rejects_dependency_or_source_identity_drift(tmp_path):
    attestation, suite_lock, extractor = _fixture(tmp_path)
    ...
    attestation, suite_lock, extractor = _fixture(tmp_path / "dependency")
